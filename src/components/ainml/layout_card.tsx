...
export default function LayoutCardAinml(props: Ainml) {
  return (
    <Link
      ...
      href={`/ainml/view/${props.fileName}`}
    >
      ...
      <div className="mx-0 mt-1 h-fit w-full px-2 text-center tracking-wider">
        <p className="mb-1 font-semibold leading-4">
          <Highlight hit={props as AinmlHit} attribute="title" />
        </p>
        <p className="text-xs text-base-content">
          <Highlight hit={props as AinmlHit} attribute="category" />
        </p>
      </div>
    </Link>
  );
