...
export default function LayoutCardTechs(props: Techs) {
  return (
    <Link
      ...
      href={`/techs/view/${props.fileName}`}
    >
      ...
      <div className="mx-0 mt-1 h-fit w-full px-2 text-center tracking-wider">
        <p className="mb-1 font-semibold leading-4">
          <Highlight hit={props as TechsHit} attribute="title" />
        </p>
        <p className="text-xs text-base-content">
          <Highlight hit={props as TechsHit} attribute="category" />
        </p>
      </div>
    </Link>
  );
